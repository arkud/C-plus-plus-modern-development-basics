...
bool operator<(const pair<F, S> &first, const pair<F, H> &second) {
...
public:
    ...
    AirportCounter() {
        for (int i = 0; i < pf_airports.size(); i++) {
            pf_airports[i].first = static_cast<TAirport>(i);
            pf_airports[i].second = 0;
        }
    }
    ...
    size_t Get(TAirport airport) const {
        return pf_airports[static_cast<uint32_t>(airport)].second;
    }

    // добавить данный элемент
    void Insert(TAirport airport) {
        pf_airports[static_cast<uint32_t>(airport)].second++;
    }
    ...
    void EraseAll(TAirport airport) {
        pf_airports[static_cast<uint32_t>(airport)].second = 0;
    }
