...
struct AddressHasher {
    size_t operator()(const Address &object) const {
        size_t x = 3;
        return s_hasher(object.city) * x * x + s_hasher(object.street) * x + i_hasher(object.building);
    }
...
struct PersonHasher {
    size_t operator()(const Person &object) const {
        size_t x = 3;
        return s_hasher(object.name) * x * x * x + i_hasher(object.height) * x * x + d_hasher(object.weight) * x +
               ad_hasher(object.address);
    }

    hash<string> s_hasher;
    hash<int> i_hasher;
    hash<double> d_hasher;
    AddressHasher ad_hasher;
};
