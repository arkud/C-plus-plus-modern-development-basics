...
Money operator+(const Money &lhs, const Money &rhs) {
    return {lhs.earned_ + rhs.earned_, lhs.spent_ + rhs.spent_};
}
...
struct BulkTaxApplier {
    ...
    BulkTaxApplier(int tax_percentage) : factor(1.0 - tax_percentage / 100.0){}
...
public:
    ...
    Money Collapse(Money origin, IndexSegment segment) const {
        return {origin.earned_ * tax_.ComputeFactor() + add_.delta * segment.length(),
                origin.spent_ + spend_.delta * segment.length()};
    }
...
struct ComputeIncomeRequest : ReadRequest<Money> {
    ...
    Money Process(const BudgetManager &manager) const override {
...
vector<Money> ProcessRequests(const vector<RequestHolder> &requests) {
    vector<Money> responses;
...
void PrintResponses(const vector<Money> &responses, ostream &stream = cout) {
    for (const Money response : responses) {
        stream << response.earned_ - response.spent_ << "\n";
    }
}
...
void TestTax() {
    ...
    stringstream ss;
    ss << "5\n"
          "Earn 2010-01-01 2010-01-05 200\n"
          "PayTax 2010-01-01 2010-01-05 50\n"
          "PayTax 2010-01-01 2010-01-05 20\n"
          "PayTax 2010-01-01 2010-01-05 10\n"
          "ComputeIncome 2010-01-03 2010-01-03";
