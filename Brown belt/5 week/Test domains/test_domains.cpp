...
void TestSimple() {
    ASSERT_EQUAL(Split("a b c").front(), "a")
    ASSERT_EQUAL(Split("").size(), 0)

    {
        Domain domain("maps.me");
        ASSERT_EQUAL(*domain.GetReversedParts().begin(), "me")
        ASSERT_EQUAL(IsSubdomain(domain, domain), true)

        ASSERT_EQUAL(IsSubdomain(Domain("me"), Domain("maps.me")), false)
    }

    {
        vector<Domain> dm = {Domain("ya.ru"), Domain("m.ya.ru")};
        DomainChecker domainChecker(dm.begin(), dm.end());

        ASSERT_EQUAL(domainChecker.IsSubdomain(Domain("pets.ya.ru")), true)
    }

    {
        ...
        stringstream result;
        const vector<Domain> banned_domains = ReadDomains(banned);
        ASSERT_EQUAL(*banned_domains.begin(), Domain("me"))
        const vector<Domain> domains_to_check = ReadDomains(domain);
        PrintCheckResults(CheckDomains(banned_domains, domains_to_check), result);

        ASSERT_EQUAL(result.str(), "Bad\n");
    }
}
