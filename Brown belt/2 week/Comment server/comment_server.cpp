...
ostream &operator<<(ostream &output, const HttpResponse &resp) {
    ...
    if (resp.headers.empty() && !resp.content.empty()) {
        output << "Content-Length: " << resp.content.size() << "\n";
    }

    for (const auto&[first, last] : resp.headers) {
        output << first << ": " << last << "\n";
    }
    output << "\n";
    output << resp.content;
    ...
    HttpResponse ServeRequest(const HttpRequest &req) {
        ...
        if (req.method == "POST") {
            if (req.path == "/add_user") {
                ...
                resp.SetCode(HttpCode::Ok).AddHeader("Content-Length", to_string(response.size()))
                        .SetContent(response);
            ...
            if (req.path == "/user_comments") {
                ...
                resp.SetCode(HttpCode::Ok).AddHeader("Content-Length", "82")
                        .SetContent(
