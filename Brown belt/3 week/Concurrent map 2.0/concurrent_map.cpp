...
public:
    ...
    explicit ConcurrentMap(size_t bucket_count) : security(bucket_count), maps(bucket_count) {

    }

    WriteAccess operator[](const K &key) {
        size_t index = hasher(key) % security.size();
        return {lock_guard(security[index]), maps[index][key]};
    }

    ReadAccess At(const K &key) const {
        size_t index = hasher(key) % security.size();
        return {lock_guard(security[index]), maps[index].at(key)};
    }
